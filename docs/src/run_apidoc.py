"""Generate API reference pages for the fairmd.idp package with sphinx-apidoc."""
...
def main():
    ...
    package_dir = os.path.relpath(os.path.join(repo_root, "src", "fairmd"), source_dir)
    auto_dir = "auto_gen"
    template_dir = "_templates/apidoc"
    excluded_patterns = ["*tests*", "*init*"]

    runs = [
        (package_dir, "fairmd.idp"),
    ]
